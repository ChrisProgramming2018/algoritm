...
TEST(InsertionSortTest, InsertionSort) {
  ...
  ASSERT_EQ(insertionSort({1}), expected);
 
  expected = {1, 2};
  ASSERT_EQ(insertionSort({1, 2}), expected);
  
  expected = {1, 2};
  ASSERT_EQ(insertionSort({2, 1}), expected);

  expected = {1, 2, 3};
  ASSERT_EQ(insertionSort({1, 3, 2}), expected);

  expected = {1, 2, 3};
  ASSERT_EQ(insertionSort({3, 1, 2}), expected);

  expected = {1, 2, 3};
  ASSERT_EQ(insertionSort({1, 2, 3}), expected);

  expected = {-10, -7, -7, -6, -6, -5, -5, -4, -4, -4, -2, -1, 1, 3, 5, 5, 6, 8, 8, 10};
  ASSERT_EQ(insertionSort({-4, 5, 10, 8, -10, -6, -4, -2, -5, 3, 5, -4, -5, -1, 1, 6, -7, -6, -7, 8}), expected);

  expected = {-10, -10, -9, -7, -7, -6, -5, -2, 2, 2, 3, 3, 4, 5, 8, 8, 9, 10};
  ASSERT_EQ(insertionSort({-7, 2, 3, 8, -10, 4, -6, -10, -2, -7, 10, 5, 2, 9, -9, -5, 3, 8}), expected);

  expected = {-10, -10, -9, -6, -5, -2, -2, -1, 1, 2, 4, 4, 6, 7, 7, 8, 8, 8, 8, 9, 10, 10};
  ASSERT_EQ(insertionSort({8, -6, 7, 10, 8, -1, 6, 2, 4, -5, 1, 10, 8, -10, -9, -10, 8, 9, -2, 7, -2, 4}), expected);

  expected = {-10, -8, -6, -2, -2, -1, 1, 1, 2, 2, 3, 5, 9};
  ASSERT_EQ(insertionSort({5, -2, 2, -8, 3, -10, -6, -1, 2, -2, 9, 1, 1}), expected);

  expected = {-10, -10, -9, -9, -9, -8, -8, -6, -4, -4, -4, -2, -1, 0, 0, 0, 0, 1, 1, 2, 2, 4, 4, 5, 5, 7, 8, 8, 9, 10};
  ASSERT_EQ(insertionSort({2, -2, -6, -10, 10, 4, -8, -1, -8, -4, 7, -4, 0, 9, -9, 0, -9, -9, 8, 1, -4, 4, 8, 5, 1, 5, 0, 0, 2, -10}), expected);

  expected = {-9, -9, -9, -7, -7, -7, -4, -4, -3, -3, -3, -2, -1, 0, 1, 1, 3, 4, 4, 5, 8, 9};
  ASSERT_EQ(insertionSort({4, 1, 5, 0, -9, -3, -3, 9, 3, -4, -9, 8, 1, -3, -7, -4, -9, -1, -7, -2, -7, 4}), expected);
  ...
  ASSERT_EQ(insertionSort({427, 787, 222, 996, -359, -614, 246, 230, 107, -706, 568, 9, -246, 12, -764, -212, -484, 603, 934, -848, -646, -991, 661, -32, -348, -474, -439, -56, 507, 736, 635, -171, -215, 564, -710, 710, 565, 892, 970, -755, 55, 821, -3, -153, 240, -160, -610, -583, -27, 131}), expected);
  ...
  ASSERT_EQ(insertionSort({991, -731, -882, 100, 280, -43, 432, 771, -581, 180, -382, -998, 847, 80, -220, 680, 769, -75, -817, 366, 956, 749, 471, 228, -435, -269, 652, -331, -387, -657, -255, 382, -216, -6, -163, -681, 980, 913, -169, 972, -523, 354, 747, 805, 382, -827, -796, 372, 753, 519, 906}), expected);
  ...
  ASSERT_EQ(insertionSort({544, -578, 556, 713, -655, -359, -810, -731, 194, -531, -685, 689, -279, -738, 886, -54, -320, -500, 738, 445, -401, 993, -753, 329, -396, -924, -975, 376, 748, -356, 972, 459, 399, 669, -488, 568, -702, 551, 763, -90, -249, -45, 452, -917, 394, 195, -877, 153, 153, 788, 844, 867, 266, -739, 904, -154, -947, 464, 343, -312, 150, -656, 528, 61, 94, -581}), expected );
